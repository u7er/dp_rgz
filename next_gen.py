...
def generate_pq(maxi):
    ...
    while True:
        ...
        if test_fermat(rsa_vars[0], maxi):
...
def generate_rsa_vars(maxi):
    rsa_vars = []
    # P Q N C D
    rsa_vars.append(max_prime_rand(31))
    rsa_vars.append(rsa_vars[0] - 1)
...
def mspa(col_val, max_colors):
    ...
    numb_vertex = {key: (((random.getrandbits(31)) << 2) | col_val_shuffle[key]) for key in col_val_shuffle}
    ...
    arr_z = {key: powd(numb_vertex[key], rsa_var[key][4], rsa_var[key][2]) for key in col_val_shuffle}

    # Move to Bob Nv Dv Zv
    ndz = {key: [rsa_var[key][2], rsa_var[key][4], arr_z[key]] for key in col_val_shuffle}
    ...
    edges_from_bob = bob_rand_edges

    # Bob received Cv1 & Cv2
    Cv1 = rsa_var[edges_from_bob[0]][3]
    Cv2 = rsa_var[edges_from_bob[1]][3]

    # Bob processing Zv1 & Zv2
    Zv1 = powd(arr_z[edges_from_bob[0]], Cv1, rsa_var[edges_from_bob[0]][2])
    Zv2 = powd(arr_z[edges_from_bob[1]], Cv2, rsa_var[edges_from_bob[1]][2])

    print Zv1, "\n", Zv2
    print ndz[edges_from_bob[0]][0], ndz[edges_from_bob[1]][0]
